  ...
  return (
    <div className="card-bg-color">
      {showCheckBoxes && (
        <Space align="start" style={{ marginTop: "12px" }}>
          ...
          <div className="setting-user">
            <p className="m-0 fs-16 line-height-18 title-color">M Waqas</p>
            <p
              className="m-0 fs-12 fw-500 line-height-18"
              style={{ marginBottom: "10px", color: "#848588" }}
            >
              mr.vaqas@outlook.com
            </p>
          </div>
        </Space>
      )}
              ...
              {selected.length > 1 && (
                <>
                  <Button
                    ...
                    icon={
                      <img width={16} height={18} src={DeleteIcon} alt="icon" />
                    }
                  />
                  <Button
                    shape="circle"
                    className="icon-button download-all"
      ...
      {data.map((obj: any, index: number) => (
        <div
          key={index}
          className={`${!showCheckBoxes && "timeline-wrapper"} ${
            selectedPar.length > 0 ? "time-selected" : ""
          }`}
        ...
        >
          <span style={{ display: "inline-block" }}>
            {!showCheckBoxes && (
              <Checkbox
                onChange={(e) => handleParentClick(e, obj.id)}
                checked={selectedPar.includes(obj.id)}
              >
                <span className="fs-20 fw-600 title-color">{obj.time}</span>{" "}
                <br />
